...
runTests() {
  ...
  if [ -f "pubspec.yaml" ] && [ -d "test" ]; then
    ...
    # Get the absolute path (e.g., /c/proj/core) instead of relative (./core)
    # This ensures genhtml can always find the source files.
    fullPath=$(cd "." && pwd)
    escapedPath="$(echo $fullPath | sed 's/\//\\\//g')"
    # ### CHANGED SECTION END ###
...
runReport() {
  if [ -f "coverage/test.info" ] && ! [ "$TRAVIS" ]; then
    ...
    # 1. Fix Windows backslashes to forward slashes
    sed -i 's/\\/\//g' coverage/test.info
    
    # 2. ### NEW: Remove leading "./" from paths (e.g., SF:./core -> SF:core) ###
    sed -i 's/SF:\.\//SF:/g' coverage/test.info

    ./genhtml.perl coverage/test.info -o coverage --no-function-coverage --prefix $(pwd)
